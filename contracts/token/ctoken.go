...
type Token struct {
	...
	// Token owner address
	Owner []byte
	// TotalSupply Key for supply value
	TotalSupply int
...
//Continual Minting of coin

// Mint supply of tokens
func (t Token) MintContinual(ctx storage.Context, to interop.Hash160, amount int) bool {
	oldamount := getIntFromDB(ctx, to)
	newAmount := oldamount + amount
	storage.Put(ctx, to, newAmount)
	oldCirculation := getIntFromDB(ctx, []byte(t.CirculationKey))
	newCirculation := oldCirculation + amount
	storage.Put(ctx, []byte(t.CirculationKey), newCirculation)
	var from interop.Hash160
	runtime.Notify("Transfer", from, to, newCirculation)
	return true
}
